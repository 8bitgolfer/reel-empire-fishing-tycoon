...
A colorful, pixel-art automatic fishing game. Start on a tiny dock with a basic rod and bluegill, then grow into private lakes, deep-sea charters, legendary waters, and a complete rare-fish collection. A fish is caught automatically every three seconds while the game is open. The fishing-specific navigation includes Tycoon, Rods, Bait Shop, Anglers, Trips, and Fishdex.
...
The layout automatically switches to a complete portrait lake on phones, keeping every navigation tab, counter, the auto-fishing panel, and the stats row on screen without horizontal cropping. Desktop keeps the original wide lake layout.
...
- Offline crew earn for up to 8 hours at 35% catch value.
- Live fishing builds Angler Momentum from 1x to 2x value and is the only way to catch trophies or complete contracts.
...
- Three daily strategies let players favor safe profit, trophy hunting, or rare-fish expeditions.
- Restaurants, collectors, and strange locals offer three live-play contracts each day.
...
- The economy keeps early upgrades at full strength, stacks Angler bonuses additively, and applies diminishing returns only to extreme late-game multiplier combinations.
...
- The bottom HUD now focuses only on Coins and Caught; Fish / Min, Level, and the redundant Auto Fishing panel were removed.
...
3. Upload the changed `index.html` and any new or changed image files, then allow GitHub to replace files with matching names. For this mobile update, upload both `index.html` and `lake-background-mobile.png`.
4. Select **Commit changes**.
...
- Upgrade: 18 rods, 14 bait tiers, 14 catch-value Angler bonuses, 15 fishing spots, and 13 boats
- Billionaire endgame: maximum upgrades cost between 10 and 50 billion coins
- Trips are functional: spots unlock species and multiply value up to 100×; boats multiply value up to 200×
